...
{
    internal static class NtfsWebDavServer
    {
...
#pragma warning restore CA1416
            ...
            listener.Start();
            Console.WriteLine($"[ntfs-webdav] WebDAV at {prefix} (read-only). Use 'net use Z: {prefix}' to map. Ctrl+C to stop.");
        ...
        private static async Task HandleRequestAsync(HttpListenerContext ctx, NtfsFileSystem ntfs)
        {
            try
            {
                ...
                string ntfsPath = UrlToNtfsPath(path);
                ...
                switch (method)
                {
                    case "OPTIONS":
                        ...
                        break;
                    case "PROPFIND":
                        await HandlePropFind(ctx, ntfs, ntfsPath);
                        break;
                    case "GET":
                        await HandleGetOrHead(ctx, ntfs, ntfsPath, head: false);
                        break;
                    case "HEAD":
                        await HandleGetOrHead(ctx, ntfs, ntfsPath, head: true);
                        break;
                    default:
        ...
        private static async Task HandlePropFind(HttpListenerContext ctx, NtfsFileSystem ntfs, string ntfsPath)
        {
            ...
            if (ntfs.DirectoryExists(ntfsPath))
            {
                ...
                if (recurse)
                {
                    ...
                    foreach (var file in ntfs.GetFiles(ntfsPath))
                    {
                        ...
                        long size = 0; try { size = ntfs.GetFileLength(file); } catch { }
            ...
            {
                long size = 0; try { size = ntfs.GetFileLength(ntfsPath); } catch { }
        ...
        private static async Task HandleGetOrHead(HttpListenerContext ctx, NtfsFileSystem ntfs, string ntfsPath, bool head)
        {
            if (!ntfs.FileExists(ntfsPath)) { ctx.Response.StatusCode = 404; return; }
