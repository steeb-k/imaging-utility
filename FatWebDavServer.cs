...
{
    internal static class FatWebDavServer
    {
...
#pragma warning disable CA1416
            using var fat = new FatFileSystem(win);
#pragma warning restore CA1416
            ...
            listener.Start();
            Console.WriteLine($"[fat-webdav] WebDAV at {prefix} (read-only). Use 'net use Z: {prefix}' to map. Ctrl+C to stop.");
            ...
            try
            {
                while (!ct.IsCancellationRequested)
                {
                    ...
                    _ = Task.Run(() => HandleRequestAsync(ctx, fat));
                }
            }
        ...
        private static async Task HandleRequestAsync(HttpListenerContext ctx, FatFileSystem fat)
        {
            try
            {
                ...
                string fatPath = UrlToFatPath(path);
                ...
                switch (method)
                {
                    case "OPTIONS":
                        ...
                        break;
                    case "PROPFIND":
                        await HandlePropFind(ctx, fat, fatPath);
                        break;
                    case "GET":
                        await HandleGetOrHead(ctx, fat, fatPath, head: false);
                        break;
                    case "HEAD":
                        await HandleGetOrHead(ctx, fat, fatPath, head: true);
                        break;
                    default:
        ...
        private static string UrlToFatPath(string urlPath)
        {
        ...
        private static async Task HandlePropFind(HttpListenerContext ctx, FatFileSystem fat, string fatPath)
        {
            ...
            if (fat.DirectoryExists(fatPath))
            {
                if (!webPath.EndsWith("/")) webPath += "/";
                EmitEntry(webPath, isDir: true, size: 0, mod: null);
                if (recurse)
                {
                    foreach (var dir in fat.GetDirectories(fatPath))
                    {
                        string name = Path.GetFileName(dir.TrimEnd('\\'));
                        EmitEntry(webPath + name + "/", true, 0, null);
                    }
                    foreach (var file in fat.GetFiles(fatPath))
                    {
                        string name = Path.GetFileName(file);
                        long size = 0; try { size = fat.GetFileLength(file); } catch { }
                        EmitEntry(webPath + name, false, size, null);
                    }
                }
            }
            else if (fat.FileExists(fatPath))
            {
                long size = 0; try { size = fat.GetFileLength(fatPath); } catch { }
        ...
        private static async Task HandleGetOrHead(HttpListenerContext ctx, FatFileSystem fat, string fatPath, bool head)
        {
            if (!fat.FileExists(fatPath)) { ctx.Response.StatusCode = 404; return; }
            using var s = fat.OpenFile(fatPath, FileMode.Open, FileAccess.Read);
            ctx.Response.StatusCode = 200;
